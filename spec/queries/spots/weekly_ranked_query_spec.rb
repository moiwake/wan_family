...
RSpec.describe Spots::WeeklyRankedQuery, type: :model do
  ...
  describe "#call" do
    ...
    it "指定した引数を渡して、親クラスのcallメソッドを呼び出す" do
      expect(RankedForSpecificPeriodQuery).to have_received(:call).once.with(scope: Spot.all, parent_record: nil, like_class: "SpotFavorite", date: "days", number: period_num)
    end
  end
end
