...
RSpec.describe ImageBlobs::WeeklyRankedQuery, type: :model do
  ...
  describe "#call" do
    ...
    it "指定した引数を渡して、親クラスのcallメソッドを呼び出す" do
      expect(RankedForSpecificPeriodQuery).to have_received(:call).once.with(
        scope: scope, parent_record: parent_record, assessment_class: assessment_class, date: date, number: period_num
      )
    end
  end
end
